...
// --- 后端代理逻辑 ---
// 为了简洁，后端代理部分与上一版完全相同，实际使用时请确保这部分代码存在。
// 您可以将上一版代码中的后端部分复制到这里。
...
export async function onRequest(context) {
  ...
  if (url.pathname === "/") {
    ...
    <body class="bg-gray-50 dark:bg-gray-900">
      <div class="min-h-screen flex flex-col items-center justify-center p-4 text-gray-900 dark:text-white">
      ...
      <script>
        document.addEventListener('DOMContentLoaded', () => {
          ...
          themeToggle.addEventListener('click', () => {
            ...
            // [优化] 同时设置 LocalStorage 和 Cookie
            localStorage.setItem('theme', newTheme);
            document.cookie = \`theme=\${newTheme}; path=/; max-age=31536000\`; // Cookie有效期一年
            applyTheme(newTheme);
          });
          ...
          window.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') { e.preventDefault(); handleAccess(); }
            if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); handleAccess(); }
          ...
          // 初始化时，JS仅需根据HTML的class来设置按钮状态，无需再读取localStorage
