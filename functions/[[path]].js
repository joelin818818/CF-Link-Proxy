    ...
    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        if (node.nodeType === 1) { // ELEMENT_NODE
          ...
          if (node.matches('script, link, img, a, source, iframe, form')) {
            rewriteElement(node);
          }
          node.querySelectorAll('script, link, img, a, source, iframe, form').forEach(rewriteElement);
        }
      });
    });
  });
        ...
        if (srcset) {
            element.setAttribute('srcset', srcset.split(',').map(part => {
                ...
                if (item[0].startsWith('http')) {
                    item[0] = `${this.proxyPrefix}${item[0]}`;
                }
                return item.join(' ');
            }).join(', '));
        }
...
async function processProxyRequest(incomingRequest) {
    ...
    let actualUrlStr = url.pathname.substring(1);
    try {
        actualUrlStr = decodeURIComponent(actualUrlStr);
    } catch(e) {
        // malformed URI, fallback to original
    }
    ...
    <head>
        ...
        <style>
          ...
          html.dark {
            ...
            --button-bg: #3b82f6;
            --button-hover: #60a5fa;
            --header-button-bg: rgba(30, 41, 59, 0.7);
            --header-button-hover: #334155;
          ...
          *,:before,:after { box-sizing: border-box; }
          ...
          .content { display: flex; flex-direction: column; align-items: center; justify-content: center; margin: auto 0; text-align: center; }
          ...
          .header-btn {
            ...
            box-shadow: 0 1px 2px 0 rgba(0,0,0,.05); transition: all .2s ease;
          ...
          .form-container { 
            width: 100%; max-width: 42rem; opacity: 0; transform: translateY(-20px);
          ...
          .sun-icon, .moon-icon, .spinner-icon { display: none; }
          ...
          kbd { 
            ...
            padding: .25rem .5rem; font-size: .75rem; margin: 0 .25rem;
          }
      ...
      <div class="main-container">
        ...
        <main class="content">
          ...
          <div class="form-container">
            <input id="url-input" type="url" value="https://example.com" placeholder="https://example.com">
